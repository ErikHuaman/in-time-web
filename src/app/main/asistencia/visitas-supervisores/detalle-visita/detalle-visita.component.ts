...
import { gradosARadianes } from '@functions/number.function';
...
export class DetalleVisitaComponent implements AfterViewInit, OnInit {
  ...
  visita: any = null; // Aquí puedes definir el tipo correcto según tu modelo
  ...
  ngAfterViewInit(): void {
    this.initMap();
    if (this.visita) {
      const sede = this.visita?.dispositivo?.sede;

      if (sede.latitud && sede.longitud) {
        this.moverMapa(sede.latitud, sede.longitud);
      }

      const latEntrada = this.visita?.latitudEntrada;
      const lonEntrada = this.visita?.longitudEntrada;
  ...
  centrarEntrada() {
    if (this.markerIn) {
      ...
      this.msg.warn('No se encontraron la marcación de entrada.');
    }
  }
  centrarSalida() {
    if (this.markerOu) {
      ...
      this.msg.warn('No se encontraron la marcación de salida.');
    }
  }
}
